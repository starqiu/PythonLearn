...
def anlysis_area_avg(province, cur_month, conn):
    data = pd.read_sql_query(
            'select area, avg(price) as avg from anjuke_ershou_house_%s_%s GROUP BY area' % (province, cur_month), conn)
...
def get_all_data(province, cur_month, conn):
    data = pd.read_sql_query(
            'select area,town,estate,price  from anjuke_ershou_house_%s_%s limit 1000' % (province, cur_month), conn)
    data['area'] = [area.encode('utf8') for area in data['area']]
